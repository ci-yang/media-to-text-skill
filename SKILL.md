...
"$PYTHON" -c "import mlx_whisper" 2>/dev/null && echo "✅ mlx-whisper" || echo "❌ mlx-whisper → $HOME/.claude/.venv/bin/pip install mlx-whisper"
"$PYTHON" -c "import opencc"      2>/dev/null && echo "✅ opencc"      || echo "❌ opencc → $HOME/.claude/.venv/bin/pip install opencc-python-reimplemented"
```
...
用 Whisper 跑音訊前 30 秒片段，不指定語言，讀取 `language` 欄位：

```bash
$HOME/.claude/.venv/bin/python - "$OUTPUT_DIR/audio.wav" <<'PYEOF'
...
```bash
$HOME/.claude/.venv/bin/python - "$OUTPUT_DIR/audio.wav" "$OUTPUT_DIR" <<'PYEOF'
import sys, json
import mlx_whisper
import opencc
...
result = mlx_whisper.transcribe(
    audio_path,
    path_or_hf_repo="mlx-community/whisper-large-v3-turbo",
    language="{detected_lang}",  # e.g. "en"
    initial_prompt="{根據語言和內容調整的提示}",
    condition_on_previous_text=False,
    word_timestamps=True,
    verbose=False,
)
